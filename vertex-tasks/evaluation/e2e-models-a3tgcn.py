...
class RecurrentGCN(torch.nn.Module):
    def __init__(self, node_features, hidden_features):
        ...
        self.recurrent = A3TGCN(in_channels=node_features, out_channels=hidden_features, periods=1)
...
for _ in range(args.trials):
    model = RecurrentGCN(num_features, args.units).to(device)
    ...
    for _ in range(args.epochs):
        ...
        for time, snapshot in enumerate(train_dataset):
            h, y_hat = model(snapshot.x[:,:,None], snapshot.edge_index, snapshot.edge_attr, h)
